Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/11c5294b04528024880fe466cab29931"
{
	Properties
	{
		...
		_Saturation_gmod_toolgun ("Saturation", Range(-1, 10)) = 0
		...
		[DoNotLock]_LightingCap_gmod_toolgun ("Max Brightness--{condition_showS:(_LightingCapEnabled==1)}", Range(0, 10)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _LightingCap_gmod_toolgun;
			...
			float _Saturation_gmod_toolgun;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, _LightingCap_gmod_toolgun);
					poiLight.indirectColor = min(poiLight.indirectColor, _LightingCap_gmod_toolgun);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_gmod_toolgun) * hueShiftAlpha.b);
