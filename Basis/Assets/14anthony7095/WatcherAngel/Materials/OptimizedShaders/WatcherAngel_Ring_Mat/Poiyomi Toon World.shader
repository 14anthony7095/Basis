Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/5670110760cfa144ea5ecc47ecd2624d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, 0.0, 0.0));
				float3 indirectColor = 0;
				#ifdef UNITY_PASS_FORWARDBASE
				indirectColor = ShadeSH9_wrapped(lerp(poiMesh.normals[0], poiMesh.normals[1], 0.0), 0.0) * poiLight.occlusion;
				#endif
				directColor = lerp(directColor, dot(directColor, float3(0.299, 0.587, 0.114)), 0.0); // Duplicated from Lightdata due to recreating the light colour
				indirectColor = lerp(indirectColor, dot(indirectColor, float3(0.299, 0.587, 0.114)), 0.0); // Ditto^
				...
				directColor *= saturate(poiLight.rampedLightMap + 1 - 1.0) * float4(1,1,1,1);
				float3 finalWrap = directColor + indirectColor;
				if (1.0)
				{
					finalWrap = clamp(finalWrap, 0.284, 1.0);
				}
				else
				{
					finalWrap = max(finalWrap, 0.284);
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0 == 3)
				{
					poiLight.directColor = max(poiLight.directColor, 0.284);
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor * min(10000, (0.284 * rcp(calculateluminance(poiLight.directColor)))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor * min(10000, (0.284 * rcp(calculateluminance(poiLight.indirectColor)))));
				}
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.09441278,0,0.09441278,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.09441278,0,0.09441278,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
