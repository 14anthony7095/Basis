Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/d2cec1539367ae64e82f7084ad00dea7"
{
	Properties
	{
		...
		_Saturation_nanextraspoi82E1Opaque ("Saturation", Range(-1, 10)) = 0
		...
		[ThryDecalPositioning(_DecalTexture_nanextraspoi82E1Opaque, _DecalTextureUV, _DecalPosition, _DecalRotation, _DecalScale, _DecalSideOffset)]
		...
		[sRGBWarning(true)]_DecalTexture_nanextraspoi82E1Opaque ("Decal--{reference_properties:[_DecalTexturePan, _DecalTextureUV, _Decal0MaskChannel]}", 2D) = "white" { }
		...
		[DoNotLock]_LightingCap_nanextraspoi82E1Opaque ("Max Brightness--{condition_showS:(_LightingCapEnabled==1)}", Range(0, 10)) = 1
		...
		_UVTileDissolveAlpha_Row0_1_nanextraspoi82E1Opaque ("Row 0 Column 1", Range(-1, 1)) = 0
		_UVTileDissolveAlpha_Row0_2_nanextraspoi82E1Opaque ("Row 0 Column 2", Range(-1, 1)) = 0
		...
		_EmissionStrength_nanextraspoi82E1Opaque ("Emission Strength", Range(0, 20)) = 0
		...
		_EmissionStrength1_nanextraspoi82E1Opaque ("Emission Strength", Range(0, 20)) = 0
		...
		_EmissiveBlink_Velocity1_nanextraspoi82E1Opaque ("Emissive Blink Velocity", Float) = 4
		...
		_EmissionStrength2_nanextraspoi82E1Opaque ("Emission Strength", Range(0, 20)) = 0
		...
		_EmissiveBlink_Velocity2_nanextraspoi82E1Opaque ("Emissive Blink Velocity", Float) = 4
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _LightingCap_nanextraspoi82E1Opaque;
			...
			float _Saturation_nanextraspoi82E1Opaque;
			...
			sampler2D _DecalTexture_nanextraspoi82E1Opaque;
			...
			float4 _DecalTexture_nanextraspoi82E1Opaque_ST;
			...
			float _UVTileDissolveAlpha_Row0_1_nanextraspoi82E1Opaque;
			float _UVTileDissolveAlpha_Row0_2_nanextraspoi82E1Opaque;
			...
			float _EmissionStrength_nanextraspoi82E1Opaque;
			...
			float _EmissionStrength1_nanextraspoi82E1Opaque;
			...
			float _EmissiveBlink_Velocity1_nanextraspoi82E1Opaque;
			...
			float _EmissionStrength2_nanextraspoi82E1Opaque;
			...
			float _EmissiveBlink_Velocity2_nanextraspoi82E1Opaque;
			...
			VertexOut vert(appdata v)
			{
				...
				if (1.0 && 1.0)
				{
					...
					isDiscardedFromDissolve += (dissolveUdim.y >= 0 && dissolveUdim.y < 1) * dot(float4(0.0, _UVTileDissolveAlpha_Row0_1_nanextraspoi82E1Opaque, _UVTileDissolveAlpha_Row0_2_nanextraspoi82E1Opaque, 0.0), xMaskDissolve);
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					...
					float distanceFadeMultiplier = lerp(_AlphaDistanceFadeMinAlpha, 1.0, smoothstep(_AlphaDistanceFadeMin, _AlphaDistanceFadeMax, distance(position, poiCam.worldPos)));
					if (0.0 > 0)
					{
			...
			struct PoiDecal
			{
				...
				float4 m_DecalTexture_nanextraspoi82E1Opaque_ST;
				...
				void SampleDecal(sampler2D decalTexture, in PoiMods poiMods, in PoiLight poiLight, in PoiMesh poiMesh, in PoiCam poiCam, float2 scaleMultiplier = float2(1, 1))
				{
					...
					float4 dduv = any(fwidth(uv) > .5) ? 0.001 : float4(ddx(uv) * m_DecalTexture_nanextraspoi82E1Opaque_ST.x, ddy(uv) * m_DecalTexture_nanextraspoi82E1Opaque_ST.y);
					decalColor = tex2D(decalTexture, poiUV(uv, m_DecalTexture_nanextraspoi82E1Opaque_ST) + m_DecalTexturePan * _Time.x, dduv.xy, dduv.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
					decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb, m_DecalHueShift + poiLight.nDotVCentered * m_DecalHueAngleStrength, m_DecalHueShiftSpeed, m_DecalHueShiftColorSpace, m_DecalHueShiftSelectOrShift);
					decalColor.a *= decalMask[m_DecalMaskChannel] * applyTilingClipping(m_DecalTiled, uv);
				}
				void SampleDecalNoAlpha(sampler2D decalTexture, in PoiMods poiMods, in PoiLight poiLight, in PoiMesh poiMesh, in PoiCam poiCam, float2 scaleMultiplier = float2(1, 1))
				{
					uv = decalUV(m_DecalTextureUV, m_DecalPosition, m_DecalRotation + decalRotation, m_DecalRotationSpeed, decalScale * scaleMultiplier, m_DecalSideOffset +sideMod, m_DecalDepth, m_DecalSymmetryMode, m_DecalMirroredUVMode, poiMesh, poiCam);
					float4 dduv = any(fwidth(uv) > .5) ? 0.001 : float4(ddx(uv) * m_DecalTexture_nanextraspoi82E1Opaque_ST.x, ddy(uv) * m_DecalTexture_nanextraspoi82E1Opaque_ST.y);
					decalColor.rgb = tex2D(decalTexture, poiUV(uv, m_DecalTexture_nanextraspoi82E1Opaque_ST) + m_DecalTexturePan * _Time.x, dduv.xy, dduv.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a).rgb;
					decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb, m_DecalHueShift + poiLight.nDotVCentered * m_DecalHueAngleStrength, m_DecalHueShiftSpeed, m_DecalHueShiftColorSpace, m_DecalHueShiftSelectOrShift);
					decalColor.a *= decalMask[m_DecalMaskChannel] * applyTilingClipping(m_DecalTiled, uv);
				}
				...
				void SampleDecalChannelSeparation(sampler2D decalTexture, in PoiMods poiMods, in PoiLight poiLight, in PoiMesh poiMesh, in PoiCam poiCam)
				{
					...
					float4 dduvSample0 = any(fwidth(uvSample0) > .5) ? 0.001 : float4(ddx(uvSample0) * m_DecalTexture_nanextraspoi82E1Opaque_ST.x, ddy(uvSample0) * m_DecalTexture_nanextraspoi82E1Opaque_ST.y);
					float4 dduvSample1 = any(fwidth(uvSample1) > .5) ? 0.001 : float4(ddx(uvSample1) * m_DecalTexture_nanextraspoi82E1Opaque_ST.x, ddy(uvSample1) * m_DecalTexture_nanextraspoi82E1Opaque_ST.y);
					float4 sample0 = tex2D(decalTexture, poiUV(uvSample0, m_DecalTexture_nanextraspoi82E1Opaque_ST) + m_DecalTexturePan * _Time.x, dduvSample0.xy, dduvSample0.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
					float4 sample1 = tex2D(decalTexture, poiUV(uvSample1, m_DecalTexture_nanextraspoi82E1Opaque_ST) + m_DecalTexturePan * _Time.x, dduvSample1.xy, dduvSample1.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalTexture_nanextraspoi82E1Opaque_ST = _DecalTexture_nanextraspoi82E1Opaque_ST;
				...
				Decal0.m_DecalRotation = 5.44;
				Decal0.m_DecalScale = float4(0.027,0.027,1,0);
				Decal0.m_DecalSideOffset = float4(0,0,0,0);
				Decal0.m_DecalPosition = float4(0.4371703,0.5484864,0,0);
				...
				if (!0.0)
				{
					...
					if (!0.0)
					{
						Decal0.SampleDecal(_DecalTexture_nanextraspoi82E1Opaque, poiMods, poiLight, poiMesh, poiCam);
					}
					#else
				...
				Decal1.m_DecalRotation = 150.0;
				Decal1.m_DecalScale = float4(0.06776331,0.06776331,0,0);
				Decal1.m_DecalSideOffset = float4(0,0,0,0);
				Decal1.m_DecalPosition = float4(0.4186,0.9218652,0,0);
				...
				Decal2.m_DecalRotation = 16.083;
				Decal2.m_DecalScale = float4(0.0677668,0.0677668,1,0);
				Decal2.m_DecalSideOffset = float4(0,-0.001428865,0,0);
				Decal2.m_DecalPosition = float4(0.0848565,0.938422,0,0);
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				if (1.0)
				{
					...
					(udim.x >= 2 && udim.x < 3),
					(udim.x >= 3 && udim.x < 4));
					da += (udim.y >= 0 && udim.y < 1) * dot(float4(0.0, _UVTileDissolveAlpha_Row0_1_nanextraspoi82E1Opaque, _UVTileDissolveAlpha_Row0_2_nanextraspoi82E1Opaque, 0.0), xMask);
				...
				switch(3.0)
				{
					default: // Basic (case 1)
					{
						...
						edgeAlpha = remapClamped(da + 0.041, da, noise) * (1 - dissolveAlpha);
						break;
					}
					case 2: // Point to Point
					{
						...
						if (1.0 != 1)
						{
							float3 pos = 1.0 == 0 ? poiMesh.localPos.rgb : poiMesh.vertexColor.rgb;
					...
					case 3: // Spherical
					{
						...
						float currentDistance = lerp(0, 0.69, dissolveAlpha);
						float fragDistance = distance(float4(0,0,0,1), poiMesh.localPos.xyz);
						float normalizedDistance;
						normalizedDistance = (fragDistance - currentDistance) / (0.69 + 0.0001) - dissolveDetailNoise * dds;
						if (0.0)
						{
							dissolveAlpha = (normalizedDistance > 0) ? 1 : 0;
							edgeAlpha = smoothstep(0.041 + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if (0.041 || (3.0 == 2 && 0.1 != 0))
				{
					#if defined(PROP_DISSOLVEEDGEGRADIENT) || !defined(OPTIMIZER_ENABLED)
					edgeColor = _DissolveEdgeGradient.Sample(sampler_MainTex, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.2158605,0,1).rgb, 0.0), float4(1,0.2158605,0,1).a);
					#else
					edgeColor = float4(poiThemeColor(poiMods, float4(1,0.2158605,0,1).rgb, 0.0), float4(1,0.2158605,0,1).a);
					#endif
					#ifndef POI_SHADOW
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0 = _EmissionStrength_nanextraspoi82E1Opaque;
				float3 emissionColor0 = 0;
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(-0.99,0,0,0), 0.0, float4(1,1,0,0), 0.0, _EmissionAL0Enabled);
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, 0.0, 0.0, float4(0,0,0,0), _EmissionAL0Enabled, 0.1);
				...
				if (3.0 > 0)
				{
					emissionMask0 = maskBlend(emissionMask0, poiMods.globalMask[3.0 - 1], 2.0);
				}
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__1 = _EmissionStrength1_nanextraspoi82E1Opaque;
				...
				emissionColor0__1 = emissionTex__1.rgb * lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, 0.0);
				emissionAlpha__1 = emissionTex__1.a;
				#else
				emissionColor0__1 = lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0__1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, poiUV(poiMesh.uv[0.0], _EmissionScrollingCurve_ST__1) + (dot(pos__1, float4(0,-10,0,0).xyz) * 2.0) + _Time.x * 1.0).r;
						#endif
					}
					else
					{
						emissionStrength0__1 *= calculateScrollingEmission__1(float4(0,-10,0,0).xyz, 1.0, 2.0, 2.0, 0.0, pos__1);
					}
				}
				if (1.0)
				{
					emissionStrength0__1 *= calculateBlinkingEmission__1(0.0, 1.0, _EmissiveBlink_Velocity1_nanextraspoi82E1Opaque, 0.0);
				}
				if(0.0)
				{
			...
			float3 applyEmission__2(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__2 = _EmissionStrength2_nanextraspoi82E1Opaque;
				...
				emissionColor0__2 = emissionTex__2.rgb * lerp(1, poiFragData.baseColor, mixBaseColor__2).rgb * poiThemeColor(poiMods, float4(1,1,0,1).rgb, 0.0);
				emissionAlpha__2 = emissionTex__2.a;
				#else
				emissionColor0__2 = lerp(1, poiFragData.baseColor, mixBaseColor__2).rgb * poiThemeColor(poiMods, float4(1,1,0,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE2) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0__2 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve2, _MainTex, poiUV(poiMesh.uv[0.0], _EmissionScrollingCurve_ST__2) + (dot(pos__2, float4(0,-10,0,0).xyz) * 2.0) + _Time.x * 1.0).r;
						#endif
					}
					else
					{
						emissionStrength0__2 *= calculateScrollingEmission__2(float4(0,-10,0,0).xyz, 1.0, 2.0, 2.0, 0.0, pos__2);
					}
				}
				if (1.0)
				{
					emissionStrength0__2 *= calculateBlinkingEmission__2(0.0, 1.0, _EmissiveBlink_Velocity2_nanextraspoi82E1Opaque, 4.0);
				}
				if(0.0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, _LightingCap_nanextraspoi82E1Opaque);
					poiLight.indirectColor = min(poiLight.indirectColor, _LightingCap_nanextraspoi82E1Opaque);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_nanextraspoi82E1Opaque) * hueShiftAlpha.b);
