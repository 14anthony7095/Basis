Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/82f91a8de6c2dc143ba539d0ecc08967"
{
	Properties
	{
		...
		_Saturation_CultHat ("Saturation", Range(-1, 10)) = 0
		...
		[DoNotLock]_LightingCap_CultHat ("Max Brightness--{condition_showS:(_LightingCapEnabled==1)}", Range(0, 10)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float shadowStrength = 0.8 * poiLight.shadowMask;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, _LightingCap_CultHat);
					poiLight.indirectColor = min(poiLight.indirectColor, _LightingCap_CultHat);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_CultHat) * hueShiftAlpha.b);
