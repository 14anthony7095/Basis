Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/3a8cada542d9af7498913ed864ffb101"
{
	Properties
	{
		...
		_Saturation_Material ("Saturation", Range(-1, 10)) = 0
		...
		[DoNotLock]_LightingCap_Material ("Max Brightness--{condition_showS:(_LightingCapEnabled==1)}", Range(0, 10)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, _LightingCap_Material);
					poiLight.indirectColor = min(poiLight.indirectColor, _LightingCap_Material);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_Material) * hueShiftAlpha.b);
