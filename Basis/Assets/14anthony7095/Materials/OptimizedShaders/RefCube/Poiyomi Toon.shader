Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/a12fe8c50ef5e0841beb9b4ee7238704"
{
	Properties
	{
		...
		_Saturation_RefCube ("Saturation", Range(-1, 10)) = 0
		...
		[DoNotLock]_LightingCap_RefCube ("Max Brightness--{condition_showS:(_LightingCapEnabled==1)}", Range(0, 10)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float4(0.3185468,0.3185468,0.3185468,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.3185468,0.3185468,0.3185468,1).rgb;
				}
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				if (0.0)
				{
					...
					float bandwidth = 1.0 == 1 ? 0.5 / aluv.y : 0.5;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, 0.8, 2.0, float4(-1,1,0,0), 1.0, 1.0);
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, 9.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, 9.0);
				#endif
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				applyALEmmissionStrength__1(poiMods, emissionStrength0__1, float4(-1,2,0,0), 0.0, float4(1,1,0,0), 0.0, 1.0);
				applyALCenterOutEmission__1(poiMods, poiLight.nDotV, emissionStrength0__1, 0.0, 0.0, float4(0,0,0,0), 1.0, 1.0);
				...
				emissionColor0__1 = emissionTex__1.rgb * lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, 10.0);
				emissionAlpha__1 = emissionTex__1.a;
				#else
				emissionColor0__1 = lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, 10.0);
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, _LightingCap_RefCube);
					poiLight.indirectColor = min(poiLight.indirectColor, _LightingCap_RefCube);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_RefCube) * hueShiftAlpha.b);
